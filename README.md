...
A Python-based utility to automatically organize files in a directory into categorized subfolders based on their file extensions. The tool offers both a pre-configured rule-based organization and an intelligent, AI-powered organization using the Gemini API.
...
1. Select **[1]**.
2. **API Key Setup:** If this is your first time, the app will ask you to enter your Gemini API Key. Paste the key you got from Google AI Studio. It will be saved locally in a `.KEY` file for future use.
3. **Select Folder:** A graphical window will pop up. Choose the folder you want to organize.
4. **AI Analysis:** The tool scans all files in that folder and sends the list of file extensions to Gemini AI.
5. **Categorization:** The AI groups similar file extensions into logical categories (e.g., separating images from documents). It displays the proposed structure.
...
1. Select **[2]**.
2. **Select Folder:** A graphical window will pop up to select your target folder.
...
Select **[3]** to print a cleanly formatted table of your current `config.json` rules so you can see exactly which extensions go into which folders.

#### Option 4: Setup API key again
Select **[4]** if you need to update or change your Gemini API key. This simply overwrites the `.KEY` file.
...
The `Pre-Config Organize` mode relies entirely on `config.json`. It looks like this:
```json
{
    "Documents": [".pdf", ".docx", ".txt", ".md"],
    "Images": [".jpg", ".png", ".svg"],
    "Programming": [".py", ".js", ".html"]
}
```
You can manually edit this file in any text editor to customize how files are grouped.
